...
nohup python covidask.py --run_mode "q_serve" --parallel --cuda --query_encoder_path "/hdd1/miyoung/covidAsk/models/denspi/1/model.pt" --query_port "$Q_PORT" > "$LOG_DIR/q_serve_$Q_PORT.log" &
...
nohup python covidask.py --run_mode "p_serve" --dump_dir "$DUMP_DIR" --query_port "$Q_PORT" --doc_port "$D_PORT" --index_port "$PORT" > "$LOG_DIR/p_serve_$PORT.log" &
